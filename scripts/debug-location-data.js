...
async function main() {
    ...
    try {
        ...
        // Check for 'ROW' type locations (maybe bays are children of Rows?)
        const rows = locations.filter(l => l.structuralType === 'ROW');
        console.log(`\nTotal locations with structuralType 'ROW': ${rows.length}`);
    } catch (err) {
