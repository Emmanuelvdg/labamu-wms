...
async function run() {
    ...
    t('1.1', 'Login Happy', await req('/auth/login', { method: 'POST', body: JSON.stringify({ email: 'admin@labamu.co.id', password: 'admin' }) }), [200, 429]);
    t('1.2', 'Login Wrong Pass', await req('/auth/login', { method: 'POST', body: JSON.stringify({ email: 'admin@labamu.co.id', password: 'wrong' }) }), [401, 429]);
    t('1.3', 'Login Unknown Email', await req('/auth/login', { method: 'POST', body: JSON.stringify({ email: 'nobody@x.com', password: 'x' }) }), [400, 401, 429]);
    ...
    if (u8.b?.id) {
        t('1.10', 'Reset Password', await req(`/settings/users/${u8.b.id}/reset-password`, { method: 'POST', body: JSON.stringify({ newPassword: 'New@5678' }) }), [200, 201]);
    ...
    // M3: API Keys
    console.log('\n--- M3: API Keys ---');
    const k31 = t('3.1', 'Create API Key', await req('/api-keys', { method: 'POST', body: JSON.stringify({ name: 'RegTest Key', scopes: ['read'] }) }), [200, 201]);
    ...
    const shelfLoc = allLocs.find(l => l.structuralType === 'SHELF'); // POSITION must be child of SHELF/BAY/ROW
    const binId = bins[0]?.id;
    const l58 = t('5.8', 'Create Location', await req('/inventory/locations', { method: 'POST', body: JSON.stringify({ name: 'RegTest Bin 99', warehouseId: DC, type: 'INTERNAL', structuralType: 'POSITION', code: 'RTEST-99', zonePriority: 25, parentId: shelfLoc?.id }) }), [200, 201]);
    if (l58.b?.id) {
    ...
    // CSV import requires capitalised column names; POSITION needs a ParentCode pointing to existing shelf
    const shelfCode = shelfLoc ? (allLocs.find(l => l.id === shelfLoc.id)?.code || '') : '';
    t('5.23', 'Import CSV', await req('/inventory/locations/import', { method: 'POST', body: JSON.stringify({ warehouseId: DC, csv: `Name,StructuralType,ParentCode\nIMP-RT-Bin,POSITION,${shelfCode}` }) }), [200, 201]);
    ...
    const prodRunId = Date.now();
    const p67 = t('6.7', 'Create Product', await req('/inventory/products', { method: 'POST', body: JSON.stringify({ sku: `RT-TST-${prodRunId}`, name: 'RegTest Product', category: 'Office Supplies', weight: 1.5, unitOfMeasure: 'Unit', type: 'Finished', isStockable: true }) }), [200, 201]);
    t('6.8', 'Create Product Dup SKU', await req('/inventory/products', { method: 'POST', body: JSON.stringify({ sku: `RT-TST-${prodRunId}`, name: 'Dup', category: 'Office Supplies', weight: 1, unitOfMeasure: 'Unit', type: 'Finished', isStockable: true }) }), [400, 409, 500]);
    if (p67.b?.id) {
    ...
    if (mouseP && binId) {
        t('7.3', 'Add Batch', await req('/inventory/batch', { method: 'POST', body: JSON.stringify({ productId: mouseP.id, locationId: binId, warehouseId: DC, quantity: 5, batchNumber: `RT-BATCH-${Date.now()}`, purchaseDate: '2026-01-01', costPerUnit: 0 }) }), [200, 201]);
    }
    if (mouseP) {
    ...
    // Pre-inject stock into binId so 9.1 (transfer) and 9.4 (scrap) have sufficient source stock
    if (mouseP && binId) {
        await req('/inventory/batch', { method: 'POST', body: JSON.stringify({ productId: mouseP.id, locationId: binId, warehouseId: DC, quantity: 20, batchNumber: `RT-PRE-STOCK-${Date.now()}`, purchaseDate: '2026-01-01', costPerUnit: 0 }) });
    }
    if (mouseP && binId && bin2) {
        ...
        t('9.3', 'Transfer Same Src/Dst', await req('/inventory/transfer', { method: 'POST', body: JSON.stringify({ productId: mouseP.id, sourceLocationId: binId, destinationLocationId: binId, quantity: 1 }) }), [201, 400, 500]);
